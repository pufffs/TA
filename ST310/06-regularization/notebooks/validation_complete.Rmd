...
### Fit models
Loess(Locally Estimated Scatterplot Smoothing): a non-parametric method used primarily for data smoothing and fitting curves to data in a flexible way. 

The span parameter takes a value between 0 and 1. It represents the fraction of the total data used in each local neighborhood when fitting a regression at a given point.

small span: The result is a less smooth, more responsive fit that can closely follow local fluctuations, which can lead to overfitting if the span is too small.
large span: The result is a smoother fit that is less responsive to local variations, which can lead to underfitting if the span is too large.
```{r}
...
OOD (out-of-distribution): Generate new test data that has a different distribution from the original data.
...
# Cross validation
$$
L(\beta) = \sum_{i=1}^{n} \left( y_i - X_i^T \beta \right)^2 + \lambda \left[ (1 - \alpha) \sum_{j=1}^{p} \beta_j^2 + \alpha \sum_{j=1}^{p} |\beta_j| \right]
$$
```{r}
library(caret)     
library(ggplot2)   
...
trainIndex <- sample(1:n, size = 0.8 * n)  #get training data index
trainData <- dataset[trainIndex, ]
testData <- dataset[-trainIndex, ] # testing is then the the whole data excludes training
