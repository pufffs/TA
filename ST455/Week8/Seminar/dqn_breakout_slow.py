...
NUM_UPDATE_COUNTS = 50000

def preprocess(x, frame_height=FRAME_HEIGHT, frame_width=FRAME_WIDTH):
        ...
        while True:
            ...
            while True:
                ...
                if self.__count % 1000 == 0:
                    print(f'Collected {self.__count} / {size} observations.')
...
class QNetwork(tf.keras.Model):
    def __init__(self, n_actions, gamma):
        ...
        # See the link here http://d2l.ai/chapter_convolutional-neural-networks/padding-and-strides.html for kernel and stride
        # self.normaliser = tf.keras.layers.Lambda(lambda x:  / 255.0)
        self.normaliser = tf.keras.layers.Lambda(lambda x: tf.cast(x, tf.float32) / 255.0)

        self.conv_layer1 = tf.keras.layers.Conv2D(filters=32, kernel_size=[8, 8], strides=[4, 4],
                                                  kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                                  activation='relu')
        self.conv_layer2 = tf.keras.layers.Conv2D(filters=32, kernel_size=[4, 4], strides=[2, 2],
                                                  kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                                  activation='relu')
        self.conv_layer3 = tf.keras.layers.Conv2D(filters=32, kernel_size=[3, 3], strides=[1, 1],
                                                  kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                                  activation='relu')
        self.flatten = tf.keras.layers.Flatten()
        self.hidden_dense_layer = tf.keras.layers.Dense(128,
                                                        kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2),
                                                        activation='relu')
    ...
    def call(self, inputs, training=None, mask=None):
        inputs = tf.reshape(inputs, [-1, FRAME_HEIGHT, FRAME_WIDTH, 4])
...
## see the definition of huber loss function here https://en.wikipedia.org/wiki/Huber_loss
...
def main():
    ...
    # '-v4' allows the agent tosee and select actions on every 4th frame instead of every frame
    ...
    replay_init_size = REPLAY_INIT_SIZE 
    ...
    for e in range(episodes):
        ...
        while update_counts < NUM_UPDATE_COUNTS:
            state = env.reset()
            is_done = False
            stacked_frames = []
            processed_state = preprocess(state)
